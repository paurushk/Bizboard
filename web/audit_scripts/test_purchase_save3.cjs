...
(async () => {
  ...
  console.log('Supplier options found:', opts);

  if (opts.length > 0) {
    await page.locator('li[role="option"]').first().click();
    console.log('Clicked supplier option');
  }
  await page.waitForTimeout(1000);
  ...
  console.log('Product input found:', await prodInput.isVisible());
  ...
  if (prodOpts.length > 0) {
    await page.locator('li[role="option"]').first().click();
    console.log('Clicked product option');
  }
  await page.waitForTimeout(1000);
