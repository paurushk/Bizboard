...
export function isEinvoiceSubmitEnabled(): boolean {
  // F1-020: honor the per-company runtime flag like every sibling (was
  // build-time only). Same shape as isEwaySubmitEnabled — runtime key OR the
  // build-time VITE_ENABLE_EINVOICE_SUBMIT fallback.
  if (features.advancedPilot) return true;
  return isRuntimeFlagEnabled('ENABLE_EINVOICE_SUBMIT') || features.einvoiceSubmit;
}
