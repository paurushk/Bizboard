...
def _run_alert_builder(name: str, company, builder) -> list[dict]:
    """R-044: one builder failure must not swallow the rest of the inbox."""
    try:
        return list(builder() or [])
    except Exception:
        logger.exception("Alert builder %s failed for company %s", name, getattr(company, "id", None))
        return []
...
def build_business_alerts(company, as_of: date | None = None) -> list[dict]:
    ...
    for name, builder in (
        ...
        ("no_sales", _no_sales),
        ("customer_concentration", _concentration),
