"""India tax pack (COMP-009).

This is the previous ``core.services.billing`` module, moved here unchanged.
``core.services.billing`` re-exports these names. ``IndiaTaxEngine`` is the
protocol adapter; call sites still use the function names.
"""
...
class IndiaTaxEngine:
    """Generic TaxEngine surface over the India functions above."""

    def round_amount(self, value):
        return q2(value)

    def apply_effective_rate(self, document, item, *, tax_enabled: bool = True):
        return apply_effective_gst_rate(document, item, tax_enabled=tax_enabled)

    def is_intra_jurisdiction(
        self,
        company_state: str,
        party_state: str,
        *,
    ...
    def compute_document_totals(self, document, items, **kwargs):
        return compute_document_totals(document, items, **kwargs)
